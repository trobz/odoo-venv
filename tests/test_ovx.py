...
FIXTURES = Path(__file__).parent / "ovx_fixtures"
...
class TestGetAddonSeries:
    def test_valid_v19(self):
        from odoo_venv.ovx_resolver import get_addon_series

        series = get_addon_series(FIXTURES / "addon_v19")
        assert series == "19.0"

    def test_valid_v17(self, tmp_path):
        from odoo_venv.ovx_resolver import get_addon_series

        # Must be in its own parent dir so the scanner doesn't pick up v19 siblings.
        addon = tmp_path / "addon_v17"
        addon.mkdir()
        (addon / "__manifest__.py").write_text('{"name": "T", "version": "17.0.1.0.0"}')
        series = get_addon_series(addon)
        assert series == "17.0"
    ...
    def test_bad_version_raises(self, tmp_path):
        ...
        # Must be in its own parent dir so no valid sibling contaminates the result.
        addon = tmp_path / "addon_bad"
        addon.mkdir()
        (addon / "__manifest__.py").write_text('{"name": "T", "version": "1.0"}')
        with pytest.raises(OdooVenvError, match="Odoo series"):
            get_addon_series(addon)
...
class TestResolveBaseVenv:
    def test_explicit_venv_matching_version(self, tmp_path):
        from odoo_venv.ovx_resolver import resolve_base_venv
...
class TestCloneVenv:
    def test_clone_produces_copy(self, tmp_path):
        from odoo_venv.ovx_resolver import clone_venv
...
class TestInstallMissingPythonDeps:
    ...
    def test_installs_missing(self, mock_freeze, mock_run, tmp_path):
        from odoo_venv.ovx_resolver import install_missing_python_deps
...
class TestBuildOdooArgv:
    def test_basic_argv(self, tmp_path):
        from odoo_venv.ovx import build_odoo_argv

        venv = tmp_path / "venv"
        addon = tmp_path / "my_addon"
        addons_path = [str(addon.parent), "/opt/odoo/addons"]
        db = "testdb"
    ...
    def test_user_dash_d_in_extras_detected(self, tmp_path):
        from odoo_venv.ovx import user_supplied_db
...
class TestMakeEphemeralDbName:
    def test_sanitization(self):
        from odoo_venv.ovx import make_ephemeral_db_name

        name = make_ephemeral_db_name(["my-addon.v2"])
        assert re.match(r"^ovx_my_addon_v2_[0-9a-f]{8}$", name), name
...
class TestRunWithDbLifecycle:
    ...
    def test_ephemeral_db_dropped_on_clean_exit(self, mock_popen, mock_run):
        from odoo_venv.ovx import run_with_db_lifecycle
...
class TestResolveAddonsPath:
    def _make_resolved_existing(self, tmp_path: Path, stored_paths: list[str]):
        from odoo_venv.ovx_resolver import ResolvedVenv
        from odoo_venv.utils import write_venv_config
    ...
    def test_merges_stored_extra_parent(self, tmp_path):
        from odoo_venv.ovx import _resolve_addons_path
    ...
    def test_no_extra_preserves_today_behavior(self, tmp_path):
        from odoo_venv.ovx import _resolve_addons_path

        resolved = self._make_resolved_existing(tmp_path, ["/stored"])
        addon = tmp_path / "my_addon"
        addon.mkdir()
        result = _resolve_addons_path(resolved, [addon])
        assert result == ["/stored", str(addon.parent)]
