...
from odoo_venv.modes import KnownIssue, Mode, ModeOverride, load_modes
...
class TestResolveOverrides:
    ...
    @patch("odoo_venv.modes.load_modes")
    def test_conservative_applies_matching_overrides(self, mock_load):
        ...
        ignore, extra = _resolve_mode_overrides("conservative", "17.0", "3.10")
        assert "urllib3==1.26.5" in ignore
        assert "urllib3==1.26.14" in extra

    @patch("odoo_venv.modes.load_modes")
    def test_marker_no_match_skips_override(self, mock_load):
        mock_load.return_value = {
            "conservative": Mode(
                description="test",
                strategy="compat",
        ...
        ignore, extra = _resolve_mode_overrides("conservative", "18.0", "3.10")
        assert ignore == []
        assert extra == []

    @patch("odoo_venv.modes.load_modes")
    def test_bleeding_edge_returns_empty(self, mock_load):
        mock_load.return_value = {
            "bleeding-edge": Mode(
                description="test",
                strategy="uncapped",
        ...
        ignore, extra = _resolve_mode_overrides("bleeding-edge", "17.0", "3.10")
        assert ignore == []
        assert extra == []

    @patch("odoo_venv.modes.load_modes")
    def test_unknown_mode_returns_empty(self, mock_load):
        mock_load.return_value = {}
        ignore, extra = _resolve_mode_overrides("nonexistent", "17.0", "3.10")
        assert ignore == []
        assert extra == []

    @patch("odoo_venv.modes.load_modes")
    def test_empty_when_always_matches(self, mock_load):
        mock_load.return_value = {
            "conservative": Mode(
                description="test",
                strategy="compat",
